...
function generateLevelVocabulary(level, seen, needed) {
  ...
  data.nouns.forEach(([article, noun, nounZh]) => {
    addGeneratedVocabularyRow(rows, seen, { level, type: "名詞", german: `${article} ${noun}`, chinese: nounZh });
  });

  data.verbs.forEach(([verb, verbZh]) => {
    addGeneratedVocabularyRow(rows, seen, { level, type: "動詞", german: verb, chinese: verbZh });
  });
